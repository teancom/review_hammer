...
- No test for empty input (empty List, empty Dictionary, empty string)
- No test for boundary values (0, -1, int.MaxValue)
- No test for error/exception paths
- No test for async/await cancellation scenarios
- Single test case where multiple distinct code paths exist
- No test for concurrent access scenarios

**DO NOT REPORT:**
- Edge cases impossible given the type system (non-nullable types)
- Edge cases handled by called methods with their own tests
- Exploratory/example tests not meant to be exhaustive
- Tests for trivial getter/setter properties
- Unit tests for well-tested external libraries
- Edge cases covered by property-based tests
- Async tests that only test happy path (edge cases in timeout)
