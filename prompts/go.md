...
Look for:
- No test for empty input (empty slice, empty string, empty map)
- No test for boundary values (0, -1, max values, single items)
- No test for error returns and error conditions
- No test for concurrent execution (if code is concurrent)
- No test for timeout and cancellation with context
- Single test case where multiple distinct behaviors exist

**DO NOT REPORT:**
- Edge cases impossible given Go's type system
...
- Compile-time type system constraints
